<template>
    <div class="signUpWraper">
        ...
        <div v-loading="lodingF">
                    ...
                    <div class="boxOne" v-if="!twoShow||hasSignUp">
                        ...
                        <el-form :model="formInfos" :rules="rules" ref="formInfos" @validate="hasvalidate" class="demo-ruleForm" label-width="80px">
                            ...
                            <el-form-item label="验证码：" prop="yzm" class="prompt" v-if="!hasSignUp||modifyPhone">
                                ...
                                <el-button type="primary" :loading="fetching" round class="yzmBtn" :disabled="smsDisable" @click="clickSms">{{smsBtnText}}</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div class="boxTwo" v-if="twoShow">
                        <h2>第二步：详细信息</h2>
                        <el-form :model="formTwoInfos" :rules="rulesTwo" ref="formTwoInfos" class="demo-ruleForm" label-width="120px">
                            <el-form-item v-for="(item,index) in twoStepsFields" :key="index" :label="item.name+'：'" :prop="item.code" v-show="!(['name','phone'].includes(item.code))">
                                <template v-if='item.code=="sex"'>
                                    <el-radio-group v-model="formTwoInfos.sex" @click.native="checkSexRadio">
                                        <el-radio label="男" :disabled="hasSignUp&&!toEdit" class="man" data-sex="男"></el-radio>
                                        <el-radio label="女" :disabled="hasSignUp&&!toEdit" class="femal" data-sex="女"></el-radio>
                                    </el-radio-group>
                                </template>
                                <template v-else>
            ...
            <div class="fixed" v-if="twoShow"></div>
            <div v-show="!successShow&&!overShow" :class="{buttonGroup:twoShow}">
    ...
    export default {
        ...
        methods: {
            checkSexRadio(e){
                let target = e.target;
                let parentNode = "";
                if(target.className=="el-radio__label"){
                    parentNode = e.target.parentNode;
                }else if(target.className=="el-radio__inner"){
                    parentNode = e.target.parentNode.parentNode;
                }
                this.formTwoInfos.sex=parentNode.dataset.sex;
            },
            nextHandler(){
                            ...
                            if(res.data.status==200){
                                res.data.data.fields.forEach(item=>{
                                    ...
                                    if(item.code!="idnumber"&&item.code!="email"){
                                        this.rulesTwo[item.code]=[{required:item.required,message:"请填写"+item.name,trigger: item.code=="sex"?'change':'blur'}]
                                ...
                                if(res.data.data.user){
                                    ...
                                    this.$set(this.formTwoInfos,"sex","男");
                                }
                            }else{
            ...
            successBoxHandler(){
                ...
                window.onbeforeunload=()=>{
                    sessionStorage.setItem("signUp",JSON.stringify({phone:this.formTwoInfos.phone}))
                }
                // this.checkPhoneCode();
            },
            overBoxHandler(){
            ...
            reloadStay(){
                ...
                this.$get(api.urls.infoStudentSignUp,{clazzId:this.staticClazzId,phone:JSON.parse(sessionStorage.signUp).phone}).then(res=>{
                    ...
                    if(res.data.status==200&&Boolean(res.data)){
                        this.twoShow=true;
                        this.overShow = false;
